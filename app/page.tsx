...
const STORAGE_DADOS = "grafaplast-dados-v3";
const STORAGE_HISTORICO = "grafaplast-historico-v3";
...
export default function SistemaPastasPreview() {
  ...
  const [dados, setDados] = useState<Cliente[]>(DADOS_INICIAIS);

  const ultimaOrdem = nova.codigo
    ? dados.find((cliente) => cliente.codigo === nova.codigo)?.pedidos.length || 0
    : 0;

  const proximaOrdem = nova.codigo ? `${ultimaOrdem + 1}°` : "";

  useEffect(() => {
    const dadosSalvos = localStorage.getItem(STORAGE_DADOS);
    const historicoSalvo = localStorage.getItem(STORAGE_HISTORICO);
  ...
  const numeroCodigo = (codigo: string) => {
    const numero = Number(codigo.split("/")[0]);
    return Number.isNaN(numero) ? 999999999 : numero;
  };

  const atualizarCodigoNovaPasta = (codigoDigitado: string) => {
    const clienteExistente = dados.find(
      (cliente) => cliente.codigo === codigoDigitado
    );

    const ultima = clienteExistente?.pedidos.length || 0;

    setNova((prev) => ({
      ...prev,
      codigo: codigoDigitado,
      ordem: codigoDigitado ? `${ultima + 1}°` : ""
    }));
  };
  ...
  const adicionar = async () => {
    ...
    const pedido: Pedido = {
      ordem: nova.ordem || proximaOrdem || "1°",
      nome: nova.nome,
    ...
    setDados((prev) => {
      ...
      if (existe) {
        return prev
          ...
          .sort((a, b) => numeroCodigo(a.codigo) - numeroCodigo(b.codigo));
      }

      return [...prev, { codigo: nova.codigo, pedidos: [pedido] }].sort(
        (a, b) => numeroCodigo(a.codigo) - numeroCodigo(b.codigo)
      );
    });

    setHistorico((prev) => [
      `📁 Nova pasta criada (${nova.codigo} - ${pedido.ordem}) - ${gerarData()}`,
      ...prev
    ]);
  ...
  const excluirPedido = (codigo: string, index: number) => {
    ...
    if (pedido) {
      setHistorico((prev) => [
        `❌ Pedido ${pedido.nome} (${pedido.ordem}) excluído da pasta ${codigo} - ${gerarData()}`,
        ...prev
      ]);
    }
  };
  ...
  const alterarStatus = (
    ...
    if (pedido) {
      setHistorico((prev) => [
        `${status === "Armazenado" ? "📥" : "📤"} Pedido ${pedido.nome} (${pedido.ordem}) ${status.toLowerCase()} - ${gerarData()}`,
        ...prev
      ]);
    }
  };
  ...
  const salvarEdicao = (codigo: string, index: number) => {
    ...
    setHistorico((prev) => [
      `✏️ Pedido ${editando.nome} (${editando.ordem}) editado - ${gerarData()}`,
      ...prev
    ]);
    ...
    <div className="min-h-screen bg-gray-100 p-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="bg-white p-6 rounded-3xl shadow-xl mb-6">
          ...
          {historicoAberto && (
            <div className="bg-gray-100 rounded-2xl p-4 mt-4 max-h-72 overflow-auto">
            ...
            <div className="bg-white p-6 rounded-3xl w-[420px]">
              ...
              <div className="flex flex-col gap-3">
                <input
                  ...
                  value={nova.codigo}
                  onChange={(e) => atualizarCodigoNovaPasta(e.target.value)}
                  onKeyDown={(e) => handleKeyDown(e, "nome")}
                />

                {nova.codigo && (
                  <div className="text-sm font-bold text-gray-600">
                    Última ordem: {ultimaOrdem}° | Próxima: {proximaOrdem}
                  </div>
                )}
                    ...
                    editando.index === index ? (
                      <div className="flex flex-col gap-3">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditando({ ...editando, formato: e.target.value })
                          }
                        />
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditando({ ...editando, cilindro: e.target.value })
                          }
                        />
                      ...
                      <>
                        ...
                        <div className="flex gap-2 flex-wrap mt-4">
                          <button
                            onClick={() =>
                              alterarStatus(cliente.codigo, index, "Armazenado")
                            }
